...
- **iOS Home Screen widget** — 🚧 **naskriptováno (2026-06-13), čeká build**: `@bacons/apple-targets` plugin (app.json) + App Group entitlement `group.com.ondrejmichalcik.kalta` + `targets/readiness-widget/` (expo-target.config.js + index.swift, WidgetKit, čte App Group UserDefaults) + `src/lib/widget.ts` (`updateReadinessWidget` přes `ExtensionStorage`, guarded) napojené na readiness screen. **Manuální kroky před buildem:** (1) `npx expo install @bacons/apple-targets`, (2) v Apple Developer založit App Group `group.com.ondrejmichalcik.kalta` + zapnout na App ID, (3) `npx expo prebuild` / EAS rebuild (nový nativní target), (4) ověřit na reálném zařízení. Editor hlásí SourceKit warning na `@main` ve standalone .swift — false positive mimo target kontext, v buildu OK.
- **iOS Share Extension** — sdílet PDF/screenshot/e-mail z Mailu nebo z appky obchodu přímo do importu nákupu. Nativní (Share Extension target + App Group). Convenience nad in-app importem ze Sprintu 8.
- **Email-forward ingestion** — forward objednávky na ingestační adresu → server-side parse (inbound email + Edge Function). Velké, magický UX.
