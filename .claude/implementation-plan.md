...
- **Use-it-up rotace** — (a) pasivní FIFO nudge na critical/soon expirace + rychlé „Use 1 / Use all" (levné, JS-only); (b) log spotřeby → reálná burn-rate (nová LWW tabulka `consumption_events`, chce reálná data).
- **iOS Home Screen widget** — readiness dny + expirující počet. Nativní (WidgetKit target + App Group entitlement + EAS rebuild), data přes sdílený App Group storage (widget neumí JS/SQLite).
