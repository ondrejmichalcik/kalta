...
- ⚠️ **(historické)** Paste password do Apple ID v iOS Settings simulátoru nefunguje — kdysi obejito přes `__DEV__`-only email/password login v `login.tsx`. **Ten dev login byl od té doby odstraněn (Phase 8 cleanup)** — poznámka je už jen historická, dev testing dnes přes TestFlight/real device.
...
- ✅ **CZ komentáře v kódu** — přeloženy do EN (supabase.ts + auth layout, 2026-06-13). Zbyly jen anglické komentáře s českými *příklady* (`lék*`) v matcheru.
...
- ✅ Orphan cleanup v storage — VYŘEŠENO (commit 65639e6): `sweep-storage` Edge Function + weekly cron (Sun 04:00 UTC) GC nereferencované objekty v `product-images` bucketu.

### Custom products rozšíření
- ✅ Upsert při přidání draftu se známým EAN — už je
- ✅ Custom products management screen — POSTAVENO (commit 65c855d): `warehouse/[warehouseId]/products.tsx` (full edit sheet name/category/shelf life + image thumbnails + search/filter).
...
- ✅ **Apple Small Business Program** — ACTIVE (approved, potvrzeno 2026-06-13). 15 % commission rate platí.
...
- ✅ Apple Small Business Program — ACTIVE (approved 2026-06-13), 15 % commission rate
...
## Sprint 7 – Readiness v2 (custom seznamy + smart match) ✅ (uzavřeno 2026-06-13)

**Uzavřeno:** code-complete + typecheck clean, committed & pushed (`c4ddc06` na origin/master), schema aplikováno na produkční Supabase (checklist tabulky + `household_members.kind`). Bugy z device testů opravené a nahrané. Device smoke-test dobíhá na posledním buildu — případné nálezy jsou běžný follow-up, ne reopen.

**Stav implementace (2026-06-13):**
- ✅ **Fáze 0** — FEMA seznam doplněn (stove, warmth, fire-extinguisher, toilet-paper; pročištěné keywordy). `emergencyKit.ts`.
- ✅ **Fáze 1** — sjednocení pohledů: `quantified` flag + `partial` stav z readiness, prohledávání `notes`, měkký category signál, třetí override `not_applicable` (vyřazený z jmenovatele) + UI sekce "Not relevant". `kitCoverage.ts`, `readiness.tsx`, `shopping.tsx`. Typecheck clean.
- ✅ **Fáze 2a** — DB schema (`checklists`, `checklist_entries`, `checklist_satisfactions`, `warehouse_checklists`) + RLS + realtime v `supabase/schema.sql`. **Aplikováno na produkční Supabase (2026-06-13).** TS typy v `database.ts`.
- ✅ **Fáze 2e** — AI smart-match: `src/lib/kitMatch.ts` (ve stylu vision.ts, BYOK + subscription gate + text-only batch), tlačítko "Smart match with AI" v `readiness.tsx` jen při `hasAnthropicKey()`, explicitní potvrzení před voláním → návrhy → apply jako force_stocked override. Typecheck clean.
- ✅ **Fáze 2b** — sync plumbing pro 4 nové tabulky (LWW model jako custom_products): `localDb` (CREATE TABLE + indexy), `localQueries` (read + keywords JSON parse), `localWrites` (CRUD + enqueue), `sync.ts` (initial pull + incremental LWW pull + GC + meta), `supabase.ts` (SQLite-first wrappery). Push je generický přes _sync_queue. `keywords` ukládáno jako JSON text (SQLite nemá array), `warehouse_checklists` má `id` PK kvůli generic delete-by-id. Typecheck clean.
- ✅ **Fáze 2c** — `computeKitCoverage` zobecněno o `entries` (default FEMA) + `pins` (pin precedence). `src/lib/checklists.ts`: `entryToKitItem`, `satisfactionsToMaps`, `seedFemaChecklist` (klon FEMA → DB + migrace legacy AsyncStorage overridů přes seed_key), `ensureSeededChecklists`.
- ✅ **Fáze 2d** — authoring UI: `checklists.tsx` (list + create + seed), `checklist/[checklistId].tsx` (coverage chipy, entry CRUD, per-entry akce: pin na položku / shopping / override / not_applicable, smart match s pinem). Odkaz "Checklists" z readiness screenu. Ruční pin řešen entry-side ("Fulfilled by…") místo z detailu položky.
- ✅ **P2P sync** — 4 nové tabulky přidány do `p2pSync.ts` (SyncBundle + export + MERGE_FIELDS + preview + import merge; satisfactions/warehouse_checklists jako insert/delete-only). `insertRow`/`replaceRow` jsou generické, takže fungují bez další úpravy. Bundle pole optional → starší bundly se pořád parsují.
- ✅ **Polish 2 — sjednocení readiness screenu** — `readiness.tsx` už NEpoužívá hardcoded FEMA + AsyncStorage; jede z DB checklistů (`ensureSeededChecklists` + `warehouse_checklists` výběr), satisfactions/piny z DB, smart match pinuje. Legacy AsyncStorage overridy se migrují při seedu. Výběr "On readiness / Not on readiness" per checklist v `checklists.tsx` (warehouse↔checklist, úroveň B): warehouse s linky ukáže jen je, bez linků fallback na všechny.
- ✅ **shopping.tsx sjednoceno** — "Refresh suggestions" bere kit gapy z aktivních DB checklistů (stejný zdroj jako readiness), ne z hardcoded FEMA + AsyncStorage. Gap `source_ref` = entry.id, konzistentní s readiness.
- ✅ **Realtime live-update** — `subscribeChecklists(warehouseId, onChange)` v supabase.ts (jeden channel, 4 tabulky, filtr `warehouse_id`, suppress vlastních echo přes isOwnEcho) napojen v readiness.tsx, checklists.tsx i checklist detailu. Tabulky jsou v `supabase_realtime` publication (schema). Edit checklistu na jednom zařízení se na druhém projeví bez nutnosti odejít/vrátit se.
- ✅ **ReadinessCard/alerts bell respektuje warehouse účel** — `warehouseTracksSupplies(warehouseId)` v `checklists.ts` (true když má aktivní checklist quantified food/water entry; default true než je nakonfigurováno). Karta se skryje a zvonek nehlásí readiness tón u warehousu, jehož aktivní seznamy nejsou food-orientované (např. "dílna"). Zapojeno v `ReadinessCard.tsx` + `WarehouseAlertsBell.tsx`.
- ✅ **Schema aplikováno + v testu** (2026-06-13). Pozn.: pozdější přídavek `household_members.kind` (household-aware add-ons, viz Post-Sprint 7 níže) vyžaduje **znovu spustit `supabase/schema.sql`** (idempotentní `add column if not exists`).

### Debt cleanup (2026-06-13)
Po Sprintu 7 projeté technické dluhy (kromě automatizovaných testů, vědomě odloženo):
- ✅ **Seed race guard** — `ensureSeededChecklists` dedupe in-flight přes `seedingInFlight` Map (víc obrazovek volá na focus naráz → jen jeden seed). Cross-device first-open race mimo scope (self-heals přes LWW dedup).
- ✅ **Legacy AsyncStorage cleanup** — `@kalta/kit_dismissed_<wid>` se po migraci do DB satisfactions smaže.
- ✅ **`visionEnabled` focus refresh** — už bylo (useFocusEffect); jen srovnána zastaralá poznámka.
- ✅ **Realtime pro household + shopping** — `subscribeHousehold` / `subscribeShopping` v supabase.ts, napojeno (readiness, HouseholdSection, shopping). Teď konzistentní s boxes/items/checklists.
- ✅ **Realtime i v glanceable kartách + bellu** — `WarehouseAlertsBell` (badge: household + shopping + checklists subs), `ReadinessCard` (household + checklists), `ShoppingCard` (shopping). Dashboard se aktualizuje živě, ne jen na focus. (Item/expiry tón bellu zůstává focus-driven — je to same-device změna; warehouse-wide item realtime kanál neexistuje, items mají box_id ne warehouse_id.)
- ✅ **Pin picker** — searchable Modal (bez cap), nahradil ActionSheetIOS limit 12 položek.
- ✅ **Plný entry editor** — `ChecklistEntrySheet.tsx` (label/group/category/quantified/keywords/rationale) + reorder přes Move up/down (swap sort_order). Nahradil label-only / keywords-only Alert.prompt.
- ✅ **Retry-with-backoff** — `scheduleAutoPush` v sync.ts: failed push → bounded exp backoff 2s→4s→…→60s (max 5 pokusů), clean drain resetuje.
- 📌 **VĚDOMÉ ROZHODNUTÍ — checklisty zůstávají LWW (last-write-wins), bez per-field conflict UI.** Konzistentní s `household_members` / `shopping_list_items`. Pro 2člennou domácnost je souběžný edit stejného entry vzácný; tichý last-writer-wins je přijatelný trade-off. Revidovat jen pokud se ukáže reálná kolize (pak by se přidaly `_changed_fields`/`_local_updated_at` sloupce + zapojení do `/conflicts`, jako boxes/items).
- 📌 **Service-role JWT v pg_cron commandu** — vědomý solo-project trade-off (zůstává). Migrační cesta až bude vůle: přesun secretu do Supabase Vault a reference přes `vault.decrypted_secrets` v cron commandu místo hardcoded JWT.

### Post-Sprint 7 fixy & doplňky (2026-06-13, z testování)
- ✅ **Paywall renew bug** — dismiss efekt v `paywall.tsx` zavíral i `lapsed` → renew modal se sám vykopl. Fix: auto-dismiss jen pro forced paywall (`canDismiss=false`); renew modal se zavírá křížkem / po koupi.
- ✅ **Post-renew sync** — `useSubscription.refresh()` při přechodu lapsed/never → active spustí `runSyncCycle` (lazy require), takže nahromaděná offline fronta se vypushne hned, ne až po restartu.
- ✅ **Profile Subscription buttony** — Renew/Restore srovnané na stejnou šířku+výšku (`actionRowPrimary` flex:1 + box model).
- ✅ **Checklist create/rename přes sheet** — `ChecklistSheet.tsx` (page-sheet, name + "Start from" Blank/Copy) nahradil Alert.prompt. Manage (Rename/Delete) přesunut do **••• header menu** (ActionSheet, jako box detail), seed bez Delete. `cloneChecklist` helper.
- ✅ **Swipe-to-delete na checklist listu** — `Swipeable` (jako box detail), jen pro custom; FEMA seed bez akce.
- ✅ **Packy jako stavební bloky vlastních seznamů** — `KIT_PACKS` (FEMA skupiny postavené z `EMERGENCY_KIT` + add-ony) v `emergencyKit.ts`; `PackPickerSheet.tsx` (modal, „Add" / „Added") dostupný z ••• → „Add from a pack…" v detailu checklistu → vloží celou skupinu přes `addAddonToChecklist` (skip dupes). JS-only.
- ✅ **Household-aware kit add-ons** — `household_members.kind` (z presetu: adult_female / infant / toddler / …) + `KIT_ADDONS` packy (feminine-hygiene, baby) v `emergencyKit.ts`. `suggestedAddons()` + `addAddonToChecklist()` v `checklists.ts`; sekce „Suggested for your household" v `HouseholdSection` s „Add to kit" → vloží pack entries do seed checklistu. **⚠️ vyžaduje znovu spustit `supabase/schema.sql`** (přidá sloupec `household_members.kind` — idempotentní `add column if not exists`). Auto-suggest podle členů (varianta B z brainstormu).
- ✅ **FEMA Sanitation: Toothbrush & toothpaste** — doplněna chybějící hygiena (`emergencyKit.ts`). Pozn.: do už naseedovaných warehousů se dostane přes ••• → „Add from a pack" → Sanitation.
- ✅ **Scan loop fix (Open Food Facts 404)** — OFF u neznámého kódu vrací HTTP 404 → `lookupByBarcode` házel výjimku → `handleScan` catch resetoval `lastBarcodeRef` → kamera re-fire stejného kódu → ~100 volání → 429. Fix: `openFoodFacts` 404 → `null` (nenalezeno, ne chyba); `handleScan` nenalezení i chybu pošle rovnou do manuálního formuláře s předvyplněným barkódem (bez alertu, **bez resetu `lastBarcodeRef`** → konec smyčky). Info „not in database" ukazuje stávající SourceBanner. Odstraněn osiřelý `runVisionFromCamera` (Path A); AI identifikace zůstává přes „add photo → Identify with AI" (Path B).

**Plán (Fáze 0/1 Metro reload; Fáze 2 vyžaduje spuštění `supabase/schema.sql`, žádný `expo run:ios`):**

> 📌 **Pozn (2026-06-13):** ⏳ markery v tomto „Plán" bloku níže jsou **historical planning artifacts** — popisují, co se mělo postavit. Vše je ✅ dodáno (viz status blok nahoře + Debt cleanup). ⏳ zde = „tohle byl plán, je zrealizováno", ne otevřená položka.

**Kontext:** Sprint 6 dodal readiness jako **dva oddělené mechanismy**, které spolu skoro nekomunikují:
1. **Dny přežití** (`computeReadiness`, `readiness.ts`) — kvantitativní, kcal/litry vs household needs.
2. **Kit checklist** (`computeKitCoverage` + `emergencyKit.ts`) — kvalitativní, 23 FEMA položek keyword-matchovaných proti `item.name`, stav stocked/on_list/purchased/missing + AsyncStorage overridy `force_stocked`/`force_missing`.

### Zjištění z validace (2026-06-13)

- **Kit checklist je slepý vůči množství.** Stačí 1 lahev a "Drinking water ✓", i když `computeReadiness` hlásí waterDays = 0,5. Dva pohledy na stejné obrazovce si protiřečí.
- **Mezery v FEMA seznamu:** chybí teplo/deka/spacák, vařič+palivo, toaletní papír, hasicí přístroj.
- **Rizikové keywordy:** `batteries` má `'aa'`/`'aaa'` (chytá "AA" jako slovo kdekoliv); `multitool` `'tool'`/`'tools'` přetahuje obecné nářadí.
- **Chybí třetí override stav "not relevant"** — uživatel nemůže říct "tahle položka pro tenhle warehouse neplatí, nezkoumej ji" (force_stocked se počítá jako splněné, force_missing proti, ani jedno nevyřadí z jmenovatele).
- **Kit je hardcoded + overridy v AsyncStorage = nesynchronizují se** mezi členy domácnosti. Pro custom seznamy musí jít do DB; je to zároveň latentní bug už dnes.
- **Auto-match kouká jen na `item.name`.** Ukládáme přitom i `category` (8 doménových), `notes`, `barcode` — `category` je nevyužitý silný signál. `brand` a syrové OFF `categories_tags` se po namapování zahazují.

### Rozhodnutí (zafixováno 2026-06-13)

- **"Warehouse je na něco jiného" se řeší na DVĚ úrovně (oboje):** per-entry skrytí (Fáze 1, levné, bez DB) + výběr seznamu(ů) per-warehouse (Fáze 2, DB). Per-entry řeší "jsem prepper, ale cash mě nezajímá"; warehouse↔checklist řeší "tahle bedna je dílna".
- **Matching = měkký category-gate + keyword.** Pro 1:1 entries (`water`, `food`) je `category` autoritativní (a sjednotí kit s `computeReadiness`). Pro sdílené kategorie (tools_safety má 7 entries) je category gate + keyword rozlišovač; když `item.category` chybí/je špatně, fallback na keyword-only. `notes` se přidá do prohledávaného textu.
- **Robustní auto-match = "návrh → potvrzení → pin"** místo tichého binárního matche. Potvrzení nízko-jisté shody → trvalý pin (uložená vazba item↔entry), odmítnutí → force_missing. Plus **učené keywordy** z pinů (distinktivní token z názvu → keyword entry, per-warehouse).
- **AI smart-match je opt-in nadstavba, ne náhrada lokálního matche.** Drží offline-first: lokální jádro (category + keyword + notes + piny) funguje vždy offline; AI jen jako tlačítko.
  - Tlačítko "Smart match" se renderuje **jen když `hasAnthropicKey()`** (BYOK už existuje ze Sprintu 3 — Keychain `kalta.anthropicKey`, `profile.tsx`, `vision.ts`).
  - **Vždy explicitní potvrzení před voláním** ("Odeslat N názvů položek Anthropicu? Použije se tvůj klíč.") — nikdy ne na pozadí.
  - Drží stejný **subscription gate jako vision** (`isCloudEnabledNow()` / `CloudFeatureDisabledError`) kvůli konzistenci paywall copy.
  - Text-only batch volání (názvy položek + labely entries → structured `tool_use` mapování s confidence), model claude-haiku jako vision. Návrhy feedují do "návrh → potvrzení → pin" flow.
- **Scope custom seznamů: per-warehouse ve v1.** FEMA = seed seznam klonovaný do warehousu (needitovaný originál). Account-level znovupoužitelné templates sdílené přes víc warehousů = pozdější enhancement.
- **v1 vědomě VYNECHÁVÁ cílové množství per-entry** ("10 konzerv fazolí") — znovu otevírá agregaci/množství; množství řeší zvlášť `computeReadiness` + `lowStock`.

### Build sekvence

**Fáze 0 — Oprava FEMA seznamu** (jen data v `emergencyKit.ts`, Metro reload, nízké riziko)
- ⏳ Přidat položky: `warmth` (deka/spacák/izofólie/teplé oblečení), `stove` (vařič+kartuše), `toilet-paper`, `fire-extinguisher`. ~23 → ~27 položek.
- ⏳ Pročistit rizikové keywordy: `batteries` (vyhodit/nahradit holé `'aa'`/`'aaa'`), zvážit `multitool` `'tool'`.

**Fáze 1 — Sjednocení pohledů + per-entry "not relevant"** (bez DB, Metro reload, střední riziko — mění sémantiku stavů)
- ⏳ `KitItem` dostane `quantified?: 'food' | 'water'` (u entries `water`, `food`).
- ⏳ `computeKitCoverage` dostane `ReadinessResult` a u quantified entries odvodí stav z dnů přežití vůči `readiness_goal_days`: `>= goal` stocked, `0 < days < goal` nový stav `partial`, `0/null` missing. `readiness.tsx` u partial renderuje "Water — 4 z 14 dní" místo holého ✓.
- ⏳ Matcher: prohledávat i `notes` vedle `name`; měkký category-gate (viz rozhodnutí výše).
- ⏳ **Třetí override `not_applicable`** do AsyncStorage override mapy. Sémantika: **vypadne z `total` i `coveredCount`** (jmenovatel se zmenší — 6/6, ne 6/27), nesmí jít do gap → shopping listu, nesmí se objevit v missing. V `readiness.tsx` sbalitelná sekce "Not relevant (N)" pro vrácení zpět.

**Fáze 2 — Custom seznamy v DB + authoring + smart match** (DB migrace, sync, největší kus, vysoké riziko)
- ⏳ **Schema** (`supabase/schema.sql`, RLS přes `is_member(warehouse_id)`):
  - `checklists (id, warehouse_id, name, is_seed bool, goal_days int null, created_at, updated_at)` — FEMA = `is_seed=true`, nemazatelný (jen skrýt).
  - `checklist_entries (id, checklist_id, label, group_name, category, keywords text[], quantified text null, rationale, sort_order)`.
  - `checklist_satisfactions (checklist_entry_id, item_id null, mode text, warehouse_id)` — `mode` ∈ `pin | force_stocked | force_missing | not_applicable`. Stávající overridy se sem překlápí.
  - `warehouse_checklists (warehouse_id, checklist_id)` — který seznam na warehouse platí (úroveň B).
- ⏳ **Sync plumbing** (~7 míst dle Sprint 6 patternu): localDb, localQueries, localWrites + enqueue, sync.ts pull, p2pSync MERGE_FIELDS, types, supabase.ts SQLite-first wrappery. 3-4 nové synced entity.
- ⏳ **Migrace stavu:** FEMA `emergencyKit.ts` → seed klon do `checklists` při prvním otevření warehousu (nebo SQL seed). Jednorázová migrace existujících `@kalta/kit_dismissed_<id>` (AsyncStorage) → `checklist_satisfactions` — **bez toho ztratíme dosud naklikané overridy + opraví se to, že se dnes nesdílí.**
- ⏳ **`computeKitCoverage` zobecnit** — místo statického `EMERGENCY_KIT` brát `entries` z DB + `satisfactions` mapu. Precedence: `force_stocked` → pin (item existuje a není prošlý) → category+keyword match → shopping fallback → `force_missing`/`not_applicable` → missing.
- ⏳ **Authoring UI** (dostupné z warehouse settings, sheet modaly dle `ItemEditSheet`/`BoxEditSheet`):
  - Správa seznamů: list checklistů warehousu + "+ New checklist" (start "from FEMA basic" klon / "Blank"), rename, smazat (seed jen skrýt).
  - Editor entry: `label` (povinné), `group`, `category`, `keywords` (tag input, prázdné = jen ruční pin), `quantified` toggle, `rationale`, reorder.
- ⏳ **Ruční pin** v detailu položky v boxu — tlačítko "Fulfills checklist item →" → picker → `checklist_satisfactions(item_id, mode='pin')`. (Authoring definuje *co*; pin přiřazuje *konkrétní položku z boxu* — dvě různá místa.)
- ⏳ **Učené keywordy** — při pinu vytáhnout distinktivní token z názvu položky → uložit jako keyword entry (per-warehouse), aby příště podobná položka chytla sama.
- ⏳ **AI smart-match** (`src/lib/kitMatch.ts` ve stylu `vision.ts`): tlačítko jen při `hasAnthropicKey()`, explicitní potvrzení, subscription gate jako vision, text-only batch → návrhy → "návrh → potvrzení → pin".
- ⏳ **Úroveň B v UI:** warehouse bez přiřazeného food-orientovaného seznamu → readiness karta skrytá nebo neutrální "Set up readiness" (stav už `ReadinessCard.tsx` má). Pozn.: per-entry `not_applicable` ovlivní jen kit checklist, NE dny přežití (ty jedou z `item.category` reálných položek) — "celý warehouse je dílna" čistě řeší až úroveň B.

### Doporučené pořadí / hodnota
- **Fáze 0 + 1** doručí ~80 % vnímané hodnoty levně (oprava seznamu + sjednocení + not-relevant), bez DB.
- **Fáze 2** je skutečná featura "vlastní seznamy" + smart match; samostatný velký kus kvůli sync + RLS + migraci. Vyřeší mimochodem i nesdílení overridů.

### Rozhodnutí: realizovat po launchi
Stejně jako Sprint 6 — post-launch, nevstupuje do launch blockingu.

---

## Sprint 8 – AI assistant layer ✅ (code-complete + schema aplikováno, v testu 2026-06-13)

**Stav (2026-06-13):** všechny 4 fáze implementované, typecheck clean.
- ✅ **Fáze 1** — `src/lib/aiProposal.ts` (`AiProposal` union + `normalizeCategory`) + `src/components/AiProposalSheet.tsx` (review s include/exclude, all kinds); smart-match (`kitMatch.ts`) vrací `AiProposal{pins}`, readiness + checklist detail přepojeny na sheet.
- ✅ **Fáze 2** — provenance: `shopping_list_items.reason` + `source='ai'` přes celý sync stack; „Why is this here?" v shopping ••• menu. **Schema změna** (reason + source check).
- ✅ **Fáze 3** — `src/lib/advisor.ts` (`analyzeReadiness` → `AiProposal{shopping}`); tlačítko „AI advisor — what should I buy?" na readiness screenu → sheet → apply jako `source='ai'`+reason.
- ✅ **Fáze 4** — `src/lib/importPurchase.ts` (`extractPurchase` image/pdf/text → `AiProposal{items}`) + screen `box/[boxId]/import.tsx` (paste text / foto/screenshot / PDF), launch z box ••• menu „Import a purchase" → review → `addItemsBatch` do bedny.
- ✅ **Schema aplikováno na produkční Supabase (2026-06-13)** — `shopping_list_items.reason` + `source='ai'` (+ dřívější `household_members.kind`). Jinak JS-only.
- 📌 Share Extension + email-forward ingestion zůstávají ve Sprintu 9 backlogu.

**Cíl:** Udělat z AI první-třídního, *důvěryhodného* pomocníka napříč appkou — postavený na jednom sdíleném primitivu **`AiProposal`** + společném review/confirm komponentu. **AI nikdy nejedná přímo:** vrátí strukturovaný návrh → appka ho přetaví do cílové podoby (řádky nákupu / piny / drafty položek) a normalizuje → user edituje a potvrdí → teprve pak zápis přes existující local-first funkce. „AI navrhuje, člověk rozhoduje." Konzistentní s `vision.ts` / `kitMatch.ts` (BYOK, subscription gate, explicitní potvrzení před voláním).

**Rozsah (zafixováno 2026-06-13):** jen páteř — `AiProposal` + review komponent, AI readiness advisor, import nákupu (multi-formát), provenance, a refactor stávajícího smart-matche na primitiv. **Odloženo na Sprint 9:** store adaptery (Rohlík atd.), use-it-up rotace + burn-rate, iOS widget, iOS Share Extension, email-forward ingestion.

**Native:** žádný nový nativní modul → Metro reload. Jediná DB změna = `shopping_list_items.reason` + `source='ai'` (nutno spustit migraci v schema.sql).

### Sdílený primitiv

```ts
type AiProposal =
  | { kind: 'shopping'; rows: { label; category; qty; reason }[] }   // advisor, gap návrhy
  | { kind: 'pins'; matches: { entryId; itemId; reason }[] }         // smart-match
  | { kind: 'items'; drafts: ItemDraft[] }                           // import nákupu
  | { kind: 'advice'; text: string };                                // čistá rada bez akce
```

- `src/lib/ai/proposal.ts` — typy + `apply*` funkce (shopping → `addShoppingItem` batch; pins → `setChecklistSatisfaction`; items → `addItemsBatch`). Normalizace AI kategorií na 8 doménových, dedup proti existujícímu seznamu.
- `src/components/AiProposalSheet.tsx` — sdílený review/confirm Modal renderující každý `kind` (editovatelné řádky / piny / drafty / text) + Apply/Discard.

### Build sekvence

**Fáze 1 — `AiProposal` primitiv + review komponent + refactor smart-matche**
- ⏳ `proposal.ts` + `AiProposalSheet.tsx`.
- ⏳ `kitMatch.ts` smart-match → vrací `AiProposal{kind:'pins'}`; readiness.tsx + checklist detail použijí `AiProposalSheet` místo bespoke Alert flow. Žádná nová AI schopnost, jen páteř + migrace existujícího (verifikovatelné).

**Fáze 2 — Provenance ("proč je to v košíku")**
- ⏳ Schema: `shopping_list_items.reason text` + `source` CHECK rozšířit o `'ai'`; `ShoppingSource` type + sync plumbing (localDb/queries/writes/sync/wrapper — jeden sloupec na existující LWW tabulce).
- ⏳ Tap na řádek shopping listu → "Why?": pro `expired`/`low_stock`/`gap` **dopočítat** z `source`+`source_ref` (bez uložení), pro `ai` ukázat uložený `reason`. Odkaz na položku/entry.

**Fáze 3 — AI readiness advisor**
- ⏳ `src/lib/ai/advisor.ts`: `analyzeReadiness(context)` → `AiProposal{kind:'shopping'}`. Kontext: household složení, goalDays, foodDays/waterDays, uncounted, kit gapy, low-stock. Structured `tool_use`, model Haiku.
- ⏳ Tlačítko na readiness screenu „What am I missing?" → potvrzení → advisor → `AiProposalSheet` → Apply do shopping listu (`source='ai'` + `reason`).

**Fáze 4 — Import nákupu (multi-formát)**
- ⏳ `src/lib/ai/importPurchase.ts`: `extractPurchase({ type: 'image' | 'pdf' | 'text', payload })` → `AiProposal{kind:'items'}`. Podle typu složí image / document (Claude nativní PDF) / text content block; prompt + výstup formát-agnostické (`{name, category, quantity, est_expiry_days}`).
  - **Vstupy:** foto papírové účtenky (Vision), screenshot online objednávky (= stejná Vision cesta), vložený text z e-mailu/webu (nejlevnější, nejpřesnější), PDF faktura (`expo-document-picker` už je).
  - Online objednávky (Rohlík/Košík/Tesco) mají čistší názvy než papír → líp se extrahují.
- ⏳ Entry point „Import purchase" (v add-items nebo na boxu) → výběr foto/screenshot/PDF/paste → review přes `AiProposalSheet` (toggly + edit, vyhození nepotravinových řádků) → `addItemsBatch` do bedny.
- 📌 Model: text/PDF zvládne levný model; chaotická papírová účtenka možná chce silnější (Sonnet) — BYOK, zvážit per-typ / volitelné.

### Známé úvahy
- AI kategorie se MUSÍ normalizovat na 8 doménových (validace v `apply*`).
- Přesnost na českých účtenkách = hlavní riziko importu; screenshot/text online objednávek je spolehlivější.
- Vše gated stejně jako Vision (BYOK klíč + `isCloudEnabledNow` + explicitní potvrzení před každým voláním).

### Rozhodnutí: realizovat po launchi
Post-launch, jako Sprint 6/7.

---

## Sprint 9 – backlog (kandidáti z brainstormu 2026-06-13, nezascopováno)

**✅ Hotovo předem (2026-06-13) — Stock identity & merge:** identita zásoby = produkt (barcode/name) + **expirace** + unit/pack/opened (zobecněno z `findMatchingItemLocal`). `addOrMergeItemLocal`/`addOrMergeItem`: shodná identita → navýší množství, jiná expirace → nová šarže (FIFO). Zapojeno do naskladňování (`add-items` Save all místo batch insert) i restocku (`RestockSheet` + předvybraná původní bedna přes `sourceItem.box_id`). Řeší duplicitní řádky stejné šarže i vracení do správné bedny. JS-only.

Odloženo ze Sprintu 8, čeká na priorizaci:
- **Store adaptery** — extensibilní `StoreAdapter` interface (generic + Rohlík/Košík/Tesco). **Reality check:** tyhle obchody nemají veřejné API pro třetí strany → strop je deeplink / „rychlé objednání" paste / formát-aware import objednávky, **ne cart-push**. Adapter to zaobalí a degraduje na generický import. Před investicí ověřit, jestli někdo nemá partner program. Reverse-engineering interního API = ToS/fragile/App-Store risk, nedoporučeno.
- **Use-it-up rotace** — ✅ **(a) hotovo (2026-06-13)**: `consumeItem`/`consumeItemLocal` (decrement / consume-all → soft-delete) + `use-soon.tsx` (expiring items, „−1" / „Use all", optimistic) + karta „Use soon (N)" na readiness. JS-only. ⏳ (b) burn-rate (log spotřeby → `consumption_events`) stále odložen (chce reálná data).
- **iOS Home Screen widget** — ✅ **hotovo + ověřeno na zařízení (2026-06-13)**: `@bacons/apple-targets` plugin (app.json) + App Group entitlement `group.com.ondrejmichalcik.kalta` + `targets/readiness-widget/` (expo-target.config.js + index.swift, WidgetKit, čte App Group UserDefaults) + `src/lib/widget.ts` (`updateReadinessWidget` přes `ExtensionStorage`, guarded) napojené na readiness screen. Widget bundle ID vyšel `com.ondrejmichalcik.kalta.widget` (z `type: 'widget'`). **Build gotcha (vyřešeno):** první EAS build failnul na „provisioning profile doesn't support App Group" — App Group se musí ručně zaškrtnout na widget App ID `…​.widget` v Apple Developer portálu, pak smazat cachované provisioning profily (`eas credentials` → Build Credentials → Provisioning Profile: Delete) a rebuildnout, aby se vygeneroval čerstvý profil. Widget commit `fba177c`. Obsah small/medium: warehouse name + days-ready (barva podle tónu) + „N expiring soon"/„Stocked", tap → `kalta://`. **Odloženo (polish later):** multi-warehouse výběr přes AppIntent config, lock-screen varianta, category breakdown.
- **iOS Share Extension** — ✅ **code-complete (2026-06-14), čeká build + device verify**: `expo-share-intent@6.1.1` (pinnuto na 6.x kvůli SDK 55; `expo install` grábl 7.x = SDK 56, srovnáno) jako config plugin v app.json (`iosActivationRules`: text + webURL + image + file/PDF, `iosAppGroupIdentifier: group.com.ondrejmichalcik.kalta` — reuse existující skupiny). `app/_layout.tsx` obalený `ShareIntentProvider` + effect routuje na `/share-import` při `hasShareIntent` (po auth/paywall guardu). Nová obrazovka `app/(app)/share-import.tsx`: přečte sdílený obsah (`useShareIntentContext`), nechá vybrat warehouse + bednu, přemapuje na `extractPurchase` input (image→upload→url, pdf→base64, text/url→text) a jede stejným `AiProposalSheet` → `addItemsBatch` flow jako in-box import ze Sprintu 8. BYOK + cloud gated, explicitní confirm před voláním. Přidán `ios.appleTeamId` do app.json (bacons + share-extension signing). **Build gotcha (stejný jako widget):** share-extension App ID (default `com.ondrejmichalcik.kalta.ShareExtension`) vznikne až při EAS buildu → pak na něm v Apple Developer zaškrtnout App Group `group.com.ondrejmichalcik.kalta`, smazat cachovaný provisioning profil (`eas credentials`) a rebuildnout. Convenience nad in-app importem.
- **Pin item ↔ kit entry z readiness dashboardu (2026-06-14)** — „Pin to an item…" existoval jen v checklist detailu; na readiness dashboardu (kde uživatel kit reálně řeší) šlo položku jen označit jako vyřešenou (force_stocked), ne spojit s konkrétním itemem. Přidáno: tři kit action sheety na readiness přepsány na opts-pattern + možnost **„Pin to an item…"** (searchable picker modal nad inventářem, `setPin` → `setChecklistSatisfaction(mode:'pin', item_id)`), u pinnuté položky navíc „Unpin (reset to auto-detect)". Pin = coverage Priority 3 → položka se počítá jako pokrytá i bez keyword matche. JS-only → OTA.
- **Audit back/navigace celé appky (2026-06-14)** — prošel jsem všechny `router.push/replace/back`. Stejnou třídu bugu (`replace(X)` kde X už je ve stacku → duplikát) měly i: **box detail** (delete box / load error / not-found → `replace(warehouse)`) a **box/new** „Back to boxes" → `replace(warehouse)`. Opraveno na guarded `router.back()` (fallback replace) — box detail i box/new jsou stack screeny nad taby, takže back() korektně popne na warehouse. **Záměrně neměněno:** `settings` leave/delete → `replace('/')` (settings je TAB, `back()` by jen přepnul tab; duplikát root je neškodný), `scan`→`replace(box)` (záměrné nahrazení scanneru), `warehouse/new`→`replace(warehouse)` a `box/new`→`replace(box)` (nahrazují vlastní obrazovku, žádný duplikát). JS-only → OTA.
- **Bugfix: back tlačítko po opakovaném přidávání do boxu (2026-06-14)** — add-items po uložení vždy dělal `router.replace(box)`. V toku box → push add-items to nahradilo add-items dalším boxem → ve stacku zůstal **box pod boxem**, takže Back z boxu vedl na duplikát boxu (a každé další přidání přidalo další). Fix: u normálního box-flow (bez restock kontextu) po uložení `router.back()` → pop add-items zpět na bednu, co už je ve stacku → Back z bedny míří na warehouse. Restock ze Shopping tabu (nese `prefillName`/`shoppingItemId`, žádná bedna pod sebou) dál `replace(box)`. Rozlišeno přes `cameFromRestock`. Box detail se obnoví přes `useFocusEffect`. JS-only → OTA.
- **Bugfix: „water running low" i s vodním filtrem (2026-06-14)** — readiness karta (`ReadinessCard`) i zvonek alertů (`WarehouseAlertsBell`) volaly `computeReadiness(items, members)` **bez** `hasWaterFilter`, takže ignorovaly filtr a hlásily vodu jako weak link (jen samotná readiness obrazovka filtr předávala). Fix: sdílený lehký detektor `hasWaterFilterInStock(items)` v `kitCoverage.ts` (stejný matcher, jen inventory — bez nutnosti načítat checklist entries/pins), použitý v kartě i zvonku. Žádná push notifikace o vodě neexistuje — šlo o tyto dva in-app indikátory. JS-only → OTA.
- **Bugfix: 2× Emergency kit (FEMA) po připojení druhé osoby (2026-06-14)** — každé zařízení seedovalo vlastní FEMA checklist s **náhodným UUID** → dva různé řádky → trvalý duplikát (komentář v kódu mylně tvrdil, že to „self-heals via LWW"). Fix: seed checklist i jeho entries teď mají **deterministické UUID** odvozené z `warehouse_id` (`deterministicId` přes SHA-256 → v5-style uuid v `checklists.ts`), takže obě zařízení vytvoří stejné PK → sync je upsertem sloučí. `createChecklist`/`createChecklistEntry` (+Local) přijímají volitelné `id`. Navíc `ensureSeededChecklists` **dedupuje existující** duplikáty (nechá nejstarší is_seed, ostatní smaže — promítne se přes sync). JS-only → OTA.
- **Product cache: editace nutrice + propagace do kusů (2026-06-14)** — na detailu produktu v product cache (`products.tsx` EditSheet) jdou u food/water produktů nastavit `energy_kcal_per_100g` + `net_weight_g`. Uložení (a) zapíše hodnoty do `custom_products` (prefill budoucích scanů) a (b) **propíše je do všech existujících kusů toho barcodu** ve warehouse (`applyProductAttributesToItems` → per-item `updateItemLocal` → enqueue → sync → druhé zařízení). `upsertCustomProduct(Local)` rozšířen o energy/net (conditional present-key zápis). Energy/kcal je vlastnost produktu, takže má smysl ji mít všude stejnou; readiness (dny jídla) se přepočítá. JS-only → OTA.
- **Bugfix: live sync mezi zařízeními nefungoval (2026-06-14)** — partnerovy změny se objevily až po restartu appky (full pull). Příčina: realtime callbacky (`subscribeBoxes`/`subscribeItems`/… v supabase.ts) po eventu jen **znovu četly lokální SQLite**, kam se vzdálená změna ještě nestáhla → uživatel neviděl nic. Fix: nový `src/lib/syncBus.ts` (`scheduleRemotePull` debounced + `onDataChanged`/`emitDataChanged`); realtime event teď spustí **pull → runSyncCycle → emitDataChanged → reload** mountnutých obrazovek. Navíc **backstop** v `(app)/_layout.tsx` (sync na každý foreground + interval 20 s), aby se data srovnala i když realtime event vypadne. `runSyncCycle` emituje dataChanged když pull přinesl řádky. `items.tsx` (bez vlastního kanálu) reloaduje přes `onDataChanged`. Server trigger `items_recalc_box` zajišťuje, že přidání položek rozhýbe i `boxes` kanál (dashboard beden). JS-only → OTA.
- **Bugfix: energetické hodnoty se ztrácely (2026-06-14)** — `energy_kcal_per_100g` (+ `net_weight_g`, `min_quantity`) u itemu nemizely vinou jednoho místa: **inkrementální pull** v `sync.ts` (řádek ~1647) měl `INSERT OR REPLACE INTO items` BEZ těchto sloupců → každý delta-pull je vynuloval (proto „po restartu zmizí i z lokálu" a zdánlivě „nesync do cloudu"). Opraveno + dohledány a opraveny VŠECHNY další item INSERT/REPLACE, které je vynechávaly: `supabase.ts` apply-path pull, a tři split/move/open cesty v `localWrites.ts` (přesun do bedny / open one / mark condition) — nově dědí energy/net/min ze zdrojového řádku. `itemMergeFields` doplněn. **Product cache:** `custom_products` rozšířen o `energy_kcal_per_100g` + `net_weight_g` (schema.sql migrace, localDb addColumnIfMissing, localWrites/supabase upsert, full+incremental pull, SAFE_FIELDS, p2p merge fields, typ `CustomProduct`); `ItemEditSheet` + `add-items` teď energy/net zapisují do cache a `add-items` z ní prefilluje při re-scanu stejného barcodu. **Pozor: nutné znovu spustit `supabase/schema.sql`** (přidává custom_products sloupce). JS-only → OTA (po aplikaci schématu).
- **In-app feedback (toasty + dialogy)** — ✅ **hotovo (2026-06-14)**: nahrazeny VŠECHNY systémové `Alert.alert`/`Alert.prompt` (179 výskytů ve 27 souborech) za themed in-app systém v `src/lib/feedback.ts` (imperativní singleton store) + `src/components/FeedbackHost.tsx` (mount v rootu nad navigátorem). API: `toast.success/error/info(msg)` (non-blocking, slide-in nahoře, auto-dismiss 2.6s, tap zavře, max 3 ve stacku), `showAlert(title, msg?, buttons?)` (drop-in za Alert.alert, themed modální karta, 2 tlačítka vedle sebe / 1× nebo 3+ stacked, destructive červeně), `showPrompt(...)` (dialog s TextInputem za Alert.prompt, onPress dostane text), `confirm({...})` (promise-based). Rozhodovací pravidlo: oznámení/chyby (1 OK) → toast, potvrzení (Cancel+akce) → showAlert, přejmenování → showPrompt. Migrace proběhla přes 7 paralelních agentů, typecheck celé appky čistý. **Doplněk (2026-06-14):** stejně nahrazeno i nativních **`ActionSheetIOS`** (12 volání v 8 souborech, readiness dashboard ap.) za themed `showActionSheet(config, callback)` — drop-in se stejným API (title/message/options/destructiveButtonIndex/cancelButtonIndex), vykreslené jako spodní bottom-sheet (destructive červeně, Cancel oddělený dole). JS-only → OTA.
- ~~**Email-forward ingestion**~~ — **ŠKRTNUTO (2026-06-13)**. Forward objednávky na ingest adresu → server-side parse (inbound email service + doména/MX + Edge Function s vlastním Anthropic klíčem). Pro 2-user appku overkill: serverová infra + jiný billing model než device-local BYOK, a **Share Extension dá ~90 % stejného užitku** (sdílení nákupu do appky) bez serveru. Místo email-forwardu preferovat Share Extension.
...
- ✅ Orphan cleanup v storage (cancelled drafts s uploadedem foto) — VYŘEŠENO `sweep-storage` Edge Function + weekly cron (commit 65639e6).
- ✅ `visionEnabled` v add-items — VYŘEŠENO: `hasAnthropicKey()` se čte přes `useFocusEffect` (re-read při každém focusu), ne jen na mount.
