...
Aktuálně: **Sprint 3A/B/C/D uzavřen** (2026-04-15) — image upload pipeline (`expo-image-picker` + `expo-image-manipulator` + `expo-file-system` `new File().arrayBuffer()` místo broken `fetch+blob`), storage RLS policies, picker v `ItemEditSheet` + `add-items.tsx`, Claude Vision identifikace přes direct client call s per-device Anthropic API klíčem v `expo-secure-store` (bezpečnější než shared Edge Function secret, každý user platí svoje volání), nový `app/(app)/profile.tsx` global settings screen pro key mgmt, Path A (auto na OFF 404) + Path B (manual "✨ Identify with AI" button), shelf life hint pod expiry pickerem (ne prefill), caching do `custom_products.typical_expiry_days`, Brother PT-P710BT print prototype (hardware-independent) — `qrLabel.ts` two-step `printToFileAsync → printAsync(uri)` pro respect embedded PDF page size (expo-print ignoruje CSS @page pro HTML), dynamic font sizing + center align, QR logo overlay (nano banana 3D crate, Python PIL auto-crop + rounded border bake, ECC-H, 7.5mm tile), napojené v `LabelModalContent` + `box/new.tsx` + matching in-app `react-native-qrcode-svg` views. Sprint 2.7 uzavřen (multi-warehouse). Next: **Brother Phase 2** (reálný print test až dorazí tiskárna) nebo Sprint 4 (push notifikace).
